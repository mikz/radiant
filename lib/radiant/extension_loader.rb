  ...
  class ExtensionLoader
    ...
    def load_extensions
      ...
      self.extensions = load_extension_roots.map do |root|
        begin
          extension_file = "#{File.basename(root).gsub(/^radiant-|-extension(-([\d\.a-z]+|[a-z\d]{12}))?$/,'')}_extension"
        ...
        roots = configuration.extensions.uniq.map do |ext_name|
          if :all === ext_name
            ...
            ext_path = all_roots.detect do |maybe_path|
              File.basename(maybe_path).gsub(/^radiant-|-extension(-([\d\.a-z]+|[a-z\d]{12}))?$/, '') == ext_name.to_s
            end
        ...
        @all_extension_roots ||= begin
          ...
          configuration.gems.inject(roots) do |paths,gem|
            paths.tap { |p| p << gem.specification.full_gem_path if gem.specification and
                            gem.specification.full_gem_path[/radiant-.*-extension(-([\d\.a-z]+|[a-z\d]{12}))?$/] }
          end
          roots.flatten
        end
      end
